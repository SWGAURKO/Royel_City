...
OnLaptopHackDone = function(success, bank)
    ...
    if bank == 'Paleto' then
        TriggerServerEvent('qb-bankrobbery:server:SetOutsideHacked')
    ...
    TriggerServerEvent('qs-smartphone:server:sendNewMail', {
        sender = Locales['mail_sender'],
        subject = bank,
        message = Locales['mail_message'],
        button = {}
    })
end
...
LaptopAnimation = function(bank)
    ...
    lib.requestAnimDict('anim@heists@ornate_bank@hack', 1500)
    lib.requestModel('hei_prop_hst_laptop', 1500)
    ...
    local time = Shared.MinigameSettings.laptop[Shared.Banks[bank].type].time
    local blocks = Shared.MinigameSettings.laptop[Shared.Banks[bank].type].blocks
    local amount = Shared.MinigameSettings.laptop[Shared.Banks[bank].type].amount
    
    local success = exports['qb-minigames']:Hacking(blocks, time)
    if success then
...
local ThermiteEffect = function(bank, index)
    ...
    Wait(1500)
    TriggerServerEvent('qb-bankrobbery:server:ThermitePtfx', bank, index)
    Wait(500)
...
StartThermite = function(bank, index) -- Globally used
    if not Shared.Banks[bank].thermite[index].hit then
        ...
        if #(pos - Shared.Banks[bank].thermite[index].coords.xyz) < 2 then
            ...
            end, Shared.MinigameSettings.thermite.timetoShow, Shared.MinigameSettings.thermite.grideSize, Shared.MinigameSettings.thermite.incorrectBlocks)
        end
    end
end
...
RegisterNetEvent('qb-bankrobbery:client:LootLocker', function(data)
    ...
    -- Check drill
    if Shared.Inventory == 'ox_inventory' and exports['ox_inventory']:Search('count', 'drill') == 0 then
        Utils.Notify(Locales['locker_missing_drill'], 'error', 3000)
        return
    elseif Shared.Inventory == 'qb' and not QBCore.Functions.HasItem('drill') then
        Utils.Notify(Locales['locker_missing_drill'], 'error', 3000)
        return
    end
...
RegisterNetEvent('thermite:UseThermite', function()
    ...
    if currentBank and Shared.Banks[currentBank].thermite then
...
CreateThread(function()
    -- Get Shared on startup
    lib.callback('qb-bankrobbery:server:GetShared', 200, function(result)
        Shared = result
    end)
    ...
    for k, v in pairs(Shared.Banks) do
        for i = 1, #v.lockers do
            exports['qb-target']:AddBoxZone('bankrobbery_locker_' .. k .. i, vector3(v.lockers[i].coords.x, v.lockers[i].coords.y, v.lockers[i].coords.z - 1.0), 0.5, 2.0, {
