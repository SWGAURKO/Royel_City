...
function IsLineOfSightClear(source, target, ignoreEntity)
    local handle, hit, endCoords, surfaceNormal, entityHit = StartShapeTestRay(source.x, source.y, source.z, target.x, target.y, target.z, -1, ignoreEntity, 0)
    local _, hit, _, _, entityHit = GetShapeTestResult(handle)

    return hit == 0 or entityHit == ignoreEntity
end
        ...
        if scoreboardOpen then
            for _, player in pairs(GetPlayersFromCoords(GetEntityCoords(PlayerPedId()), 10.0)) do
                ...
                if Config.ShowIDforALL or (playerOptin[playerId] and playerOptin[playerId].optin) then
                    ...
                    local isVisible = IsLineOfSightClear(playerCoords, cameraCoords, playerPed)
