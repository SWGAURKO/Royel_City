            ...
            QBcore.Functions.TriggerCallback('zr-multicharacter:getSkinData', function(zr_model, zr_skin)
				...
				if zr_model ~= nil then
					CreateThread(function()
						...
						elseif (zr_config.clothing=='rcore_clothing') then
							exports.rcore_clothing:setPedSkin(zr_current_char, zr_skin)	
						end
				...
				CreateThread(function()
					exports.spawnmanager:spawnPlayer({x = zr_config.StartCoords.x,y = zr_config.StartCoords.y,z = zr_config.StartCoords.z,heading = zr_config.StartCoords.w,model = ped,skipFade = true
						...
						if GetResourceState('rcore_clothing') == "started" then
							local skin = exports["rcore_clothing"]:getSkinByIdentifier(zr_data.identifier)
					
							if skin and skin.ped_model then
								zr_multichar_loadModel(skin.ped_model)
								SetPlayerModel(PlayerId(), skin.ped_model)
								SetModelAsNoLongerNeeded(skin.ped_model)
								exports.rcore_clothing:setPedSkin(PlayerPedId(), skin.skin)
							end
						elseif zr_data.skin then 
							zr_multichar_loadModel(zr_model)
...
--[[ function OpenCharacterSelection()
    exports['UPCODE-HUD']:hide(true)
    -- Rest of the character selection code...
end

AddEventHandler('OnPlayerSpawned', function()
    exports['UPCODE-HUD']:hide(false)
    -- Rest of the code after spawn...
end)
 ]]