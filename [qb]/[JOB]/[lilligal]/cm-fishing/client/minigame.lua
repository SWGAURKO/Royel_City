...
Citizen.CreateThread(function()
   while true do
      Citizen.Wait(0)
      if fishStarted or startfish or fishingReady then
...
function StartFishing(castLocation, fishingRod)
   if fishStarted then
      local findRequriedBait = GetBaitDataByQuality(currentSpot.fishQualityScaleTable[1])
      local msg = Config.Notifications["waitingbait"]["message"] .. " : " .. findRequriedBait[1].name
      Config.Notification(msg, Config.Notifications["waitingbait"]["type"])
      SendNUIMessage({
         type = 'FISHING_STARTED',
         text = msg,
         time = 5000,
      ...
      while true do
         Citizen.Wait(1)
         if IsControlJustPressed(0, 47) or IsControlJustPressed(0, 113)  then
            break
         end
         if not fishStarted or not fishingReady then
            StopAFKFishing()
