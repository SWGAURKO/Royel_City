...
HSN.Config = {
    ...
    EnableFuel = false,
    FuelSystem = 'LegacyFuel',                 -- LegacyFuel / ox-fuel / nd-fuel / frfuel / cdn-fuel / hyon_gas_station / ox-fuel
    ...
    Inventory = "ox_inventory",                --// ox_inventory //esx_default // custom // qb-inventory
...
HSN.Target = function(v, k)
    ...
    elseif v.show.InteractionHandler == 'bt-target"' then
        ...
        if v.show.marker and v.show.marker.enable then
            HSN.DrawMarker(v.coord.x, v.coord.y, v.coord.z, v.show.marker.type, v.show.marker.r, v.show.marker.g,
                v.show.marker.b, v.show.marker.rotX, v.show.marker.rotY, v.show.marker.rotZ)
        end
...
HSN.DrawMarker = function(x, y, z, mType, r, g, b, rotx, roty, rotz, conf)
    if conf ~= nil then
        DrawMarker(mType, x, y, z, 0.0, 0.0, 0.0, -90.0, 0.0, 0.0, 5.0, 1.0, 5.0, r, g, b, 100, false, true, 2, false,
            false, false, false)
        return
    end
    DrawMarker(mType, x, y, z, 0.0, 0.0, 0.0, rotx, roty, rotz, 0.3, 0.2, 0.15, r, g, b, 222, false, false, false, false,
        false, false, false)
end
...
HSN.CalculateEarnMoney = function(source, route, amount, finished, istransportjob)
    ...
    if HSN.Config.DeliverAmount.price.kind == "perofdeliver" then
        if amount then
            local random = math.random(route.revenue.min, route.revenue.max)
...
HSN.Messages = {
    client = {
        ...
        ["finishjob"] =
        "You finished the job! You should to park your vehicle to Post Op. Garage. also refunded your badge price!",
