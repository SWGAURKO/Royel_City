...
window.addEventListener("message", function (event) {
    if(event.data.status == "openbank") {
        ...
        $("#accountName1").html(event.data.information.name)
...
function populateBanking(data)
{
    ...
    $("#currentBalance4").html(data.bankbalance);
    $("#currentCashBalance4").html(data.cash);
    $("#currentBalance5").html(data.bankbalance);
...
}// Assuming you have an array of transactions with 'type' (withdraw, deposit, transfer) and 'amount' properties
const transactions = [
