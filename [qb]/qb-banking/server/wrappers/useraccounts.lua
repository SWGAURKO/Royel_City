...
function generateCurrent(cid)
    ...
    local getCurrentAccount = MySQL.query.await('SELECT * FROM mastercards WHERE citizenid = ?', { self.cid })
    ...
    rTable.generateNewCard = function(pin, scc)
        ...
        if not self.cardActive then
            ...
            MySQL.update('UPDATE mastercards SET cardnumber = ?, cardPin = ?, cardDecrypted = ?, cardActive = ?, cardLocked = ?, cardType = ? WHERE citizenid = ? AND record_id = ?', {
                cardNumber,
                pinSet,
                false,
                1,
