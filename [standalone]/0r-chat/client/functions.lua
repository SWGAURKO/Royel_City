...
function Koci.Client:TriggerServerCallback(key, payload, func)
    ...
    TriggerServerEvent("0r-chat:Server:HandleCallback", key, payload)
end
...
function onUpdateAnyNameValue()
    Koci.Client:TriggerServerCallback("0r-chat:updateNameValue", Koci.Client.Name)
end

function onUpdateMeDoPP()
    Koci.Client:TriggerServerCallback("0r-chat:updateMeDoPP", Koci.Client.MeDoPP)
end
...
function Koci.Client:LearnPlayerBalance()
    local xPlayer = self:GetPlayerData()
    if xPlayer then
        self:TriggerServerCallback("0r-chat:sendBalanceInfoMessage")
    end
end
...
function Koci.Client:RollTheDice()
    ...
    if xPlayer then
        ...
        self:TriggerServerCallback("0r-chat:RollTheDice", {
            diceValue = diceValue
        })
    end
end
...
function Koci.Client:RPS()
    ...
    if xPlayer then
        ...
        Citizen.Wait(1500)
        ClearPedTasks(PlayerPedId())
        self:TriggerServerCallback("0r-chat:RPS", {
            rpsValue = rpsValue
        })
    end
end
...
function __Thread_Writing()
    if writingThread then return end
    writingThread = true
    CreateThread(function()
        while next(WritingPlayers) ~= nil do
            local sleep = 500
            local playerCoords = GetEntityCoords(PlayerPedId())
            for writerSource, _ in pairs(WritingPlayers) do
                writerSource = tonumber(writerSource)
                local writerId = GetPlayerFromServerId(writerSource)
                if writerId ~= -1 then
                    local writerPed = GetPlayerPed(writerId)
                    local writerCoords = GetEntityCoords(writerPed)
                    if #(playerCoords - writerCoords) <= 6.0 then
                        sleep = 1
                        DrawDots(vec3(writerCoords.x, writerCoords.y, writerCoords.z + 1.0), _dot)
                    end
                end
            end
            Wait(sleep)
