...
RegisterNetEvent('qb-radio:server:consumeBattery', function(data)
    for i=1, #data do
        local id = data[i]
        ...
        if batteryData[id] == 0 then
            TriggerClientEvent('qb-radio:client:nocharge', source)
        end
    end
end)

RegisterNetEvent('qb-radio:server:rechargeBattery', function()
    local src = source
...
RegisterNetEvent('qb-radio:server:spawnobject', function(data)
    local src = source
	CreateThread(function()
        ...
        TriggerClientEvent('qb-radio:client:syncobject', -1, {
            enable = true,
            object = netobj,
            coords = data.coords,
            id = data.id,
...
RegisterNetEvent('qb-radio:server:togglejammer', function(id)
    for i=1, #jammer do
        local entity = jammer[i]
        if entity.id == id then
            jammer[i].enable = not jammer[i].enable
            TriggerClientEvent('qb-radio:client:togglejammer', -1, id, jammer[i].enable)
            break
        end
    end
end)

RegisterNetEvent('qb-radio:server:removejammer', function(id, isDamaged)
    local src = source
	CreateThread(function()
        for i=1, #jammer do
            local entity = jammer[i]
            if entity.id == id then
                ...
                TriggerClientEvent('qb-radio:client:removejammer', -1, id)
...
RegisterNetEvent('qb-radio:server:changeJammerRange', function(id, range)
    for i=1, #jammer do
        local entity = jammer[i]
        if entity.id == id then
            jammer[i].range = range
            TriggerClientEvent('qb-radio:client:changeJammerRange', -1, id, range)
            break
        end
    end
end)

RegisterNetEvent('qb-radio:server:removeallowedchannel', function(id, allowedChannels)
    for i=1, #jammer do
        local entity = jammer[i]
        if entity.id == id then
            jammer[i].allowedChannels = allowedChannels
            TriggerClientEvent('qb-radio:client:removeallowedchannel', -1, id, allowedChannels)
            break
        end
    end
end)

RegisterNetEvent('qb-radio:server:addallowedchannel', function(id, allowedChannels)
    for i=1, #jammer do
        local entity = jammer[i]
        if entity.id == id then
            jammer[i].allowedChannels = allowedChannels
            TriggerClientEvent('qb-radio:client:addallowedchannel', -1, id, allowedChannels)
            break
        end
    end
end)
...
AddEventHandler("playerDropped", function()
    ...
    for id, channel in pairs (channels) do
        if channel[tostring(plyid)] then
            ...
            TriggerClientEvent('qb-radio:client:radioListUpdate', -1, channels[id], id)
            break
        end
    end
end)

RegisterNetEvent("qb-radio:server:createdefaultjammer", function()
    if spawnedDefaultJammer then return end
    for i=1, #Shared.Jammer.default do
        local data = Shared.Jammer.default[i]
        TriggerEvent('qb-radio:server:spawnobject', {
            coords = data.coords,
            id = data.id,
            range = data.range,
...
lib.callback.register('qb-radio:server:getbatterydata', function(source)
...
lib.callback.register('qb-radio:server:getjammer', function()
    return jammer
end)
...
if Shared.UseCommand or not Shared.Inventory then
    ...
    lib.addCommand('radio', {
        ...
        TriggerClientEvent('qb-radio:client:use', source, 100)
    end)
    lib.addCommand('jammer', {
        help = 'Setup Jammer',
        params = {},
    }, function(source)
        TriggerClientEvent('qb-radio:client:usejammer', source)
    end)
    lib.addCommand('rechargeradio', {
        help = 'Recharge Radio Battery',
        params = {},
    }, function(source)
        TriggerClientEvent('qb-radio:client:recharge', source)
    end)
end
