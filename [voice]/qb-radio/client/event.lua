AddEventHandler('onResourceStart', function(resource)
    if GetCurrentResourceName() == resource then
        if Shared.Core and Framework.core.playerLoaded() then
            ...
            TriggerServerEvent('qb-radio:server:createdefaultjammer')
        end
    end
end)
...
RegisterNetEvent('qb-radio:client:use', function()
    ...
    local battery = lib.callback.await('qb-radio:server:getbatterydata', false)
...
RegisterNetEvent('qb-radio:client:usejammer', function()
    ...
    TriggerServerEvent('qb-radio:server:spawnobject', {
...
RegisterNetEvent('qb-radio:client:syncobject', function(data)
...
RegisterNetEvent('qb-radio:client:changeJammerRange', function(id, range)
...
RegisterNetEvent('qb-radio:client:removeallowedchannel', function(id, allowedChannels)
    for i=1, #Radio.jammer do
        local entity = Radio.jammer[i]
        if entity.id == id then
...
RegisterNetEvent('qb-radio:client:addallowedchannel', function(id, allowedChannels)
    for i=1, #Radio.jammer do
        local entity = Radio.jammer[i]
        if entity.id == id then
            entity.allowedChannels = allowedChannels
...
RegisterNetEvent('qb-radio:client:togglejammer', function(id, state)
    for i=1, #Radio.jammer do
        local entity = Radio.jammer[i]
        if entity.id == id then
            entity.enable = state
...
RegisterNetEvent('qb-radio:client:removejammer', function(id)
    for i=1, #Radio.jammer do
        local entity = Radio.jammer[i]
        if entity.id == id then
            entity.zone:remove()
...
RegisterNetEvent('qb-radio:client:nocharge', function()
    Radio:leaveradio()
end)

RegisterNetEvent("qb-radio:client:recharge", function()
    TriggerServerEvent('qb-radio:server:rechargeBattery')
end)
...
RegisterNetEvent('bl_bridge:client:playerLoaded', function()
    ...
    Radio:Init()
    TriggerServerEvent('qb-radio:server:createdefaultjammer')
    local jammer = lib.callback.await('qb-radio:server:getjammer', false)
    for i = 1, #jammer do
        jammer[i].entity = NetworkGetNetworkIdFromEntity(jammer[i].entity)
        TriggerEvent('qb-radio:client:syncobject', jammer[i])
    end
end)
